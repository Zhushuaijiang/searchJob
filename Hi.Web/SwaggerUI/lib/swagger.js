          ...
          if (response.apis) {
            ...
            for (_i = 0, _len = _ref.length; _i < _len; _i++) {
                ...
                if (endpoint.operations) {
                    for (_j = 0, _len1 = endpoint.operations.length; _j < _len1; _j++) {
                        operation = endpoint.operations[_j];
                        if (operation.parameters) {
                            for (_k = 0, _len2 = operation.parameters.length; _k < _len2; _k++) {
                                param = operation.parameters[_k];
                                if (param.name == "file") {
                                    //alert(param.dataType);
        ...
        if (param.paramType === 'path') {
          if (args[param.name]) {
            ...
            delete args[param.name];
              ...
              //customize code
              if (param.required)
                  //only throw exception when the parameter is "required"
                  throw "" + param.name + " is a required path param.";
              else {
                  //remove optional parameter from the reuqest
                  reg = new RegExp('&' + param.name + '=\{' + param.name + '[^\}]*\}', 'gi');
                  url = url.replace(reg, "");
                  reg = new RegExp(param.name + '=\{' + param.name + '[^\}]*\}', 'gi');
                  url = url.replace(reg, "");
                  delete args[param.name];
