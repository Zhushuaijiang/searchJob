        ...
        public void GetRequest(ref List<JobInfo> listJobInfo, string url, DataType type, int pn = 1, string kd = ".net")
        {
            ...
            switch (type)
            {
                case DataType.猎聘网:
                    ...
                    #region MyRegion
                    var ulS = response.DocumentNode.SelectNodes("//*[@id='sojob']/div[3]/div/div/div/ul/li/div");
                    ...
                    foreach (var item in ulS)
                    {
                        ...
                        titleName = item.SelectSingleNode(xpath + "/div/h3").Attributes["title"].Value;
                        infourl = item.SelectSingleNode(xpath + "/div/h3/a").Attributes["href"].Value;
                        company = item.SelectSingleNode(xpath + "/div[2]/p/a").InnerText;
                        city = "";
                        try
                        {
                            city = item.SelectSingleNode(xpath + "/div[1]/p/a").InnerText;
                        }
                        catch (Exception)
                        {
                        ...
                        date = item.SelectSingleNode(xpath + "/div[1]/p[2]/time").Attributes["title"].Value;
                        //"("+item.SelectSingleNode(xpath + "/div[1]/p[2]/time").InnerText+")";
                        salary = item.SelectSingleNode(xpath + "/div[1]/p[1]/span").InnerText;
                        salary_em ="";
                        source = "猎聘网";
                ...
                case DataType.前程无忧:
                    ...
                    for (int i = 1; i < ulS.Count; i++)
                    {
                        ...
                        var xpath = item.XPath;
                        string titleName, infourl, company, city, date, salary, salary_em, source;
                        titleName = item.SelectSingleNode(xpath + "/p/span/a").InnerText.Trim();
                        infourl = item.SelectSingleNode(xpath + "/p/span/a").Attributes["href"].Value;
                        company = item.SelectSingleNode(xpath + "/span[1]/a").InnerText;
                        city = item.SelectSingleNode(xpath + "/span[2]").InnerText;
                        date = item.SelectSingleNode(xpath + "/span[4]").InnerText;
                        salary = item.SelectSingleNode(xpath + "/span[3]").InnerText;
                        salary_em = "";
                        source = "前程无忧";
